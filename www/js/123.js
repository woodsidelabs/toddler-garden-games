...
function textToSpeech(speakStr) {
...
function shuffle() {
	...
	textToSpeech("Find the number "+targetNumber);
	document.getElementById("img_suggest").src = 'img/spot_'+targetNumber+'.png';
	//TTS.speak({ text: "Find the number "+targetNumber, locale: 'en-US' });
	//console.log("Find the number "+targetNumber);
}
...
function MemoryCards(e) {
	...
	if (e.target===testCard) {
		//TTS.speak({ text: "You found it! Good job!", locale: 'en-US' });
		textToSpeech("You found it! Good job!");
		//document.getElementById("status").innerText = "You found it! Good job!";
		setTimeout(() => { shuffle(); }, 2000);
		return;
	}
	if (e.target.alt!=='blank') {
		//TTS.speak({ text: "Sorry, try again!", locale: 'en-US' });
